...
namespace game
{
	...
	public:
		...
		const std::vector<ColumnRenderData>& generateWallRenderData(const math::Transform& playerTransform, const map::GameMap* map, rcgf::Texture* wallTexture);
	...
	private:	
		ColumnRenderData DoRayTest(const math::Transform& transform, float rayAngle, int pxCol, const map::GameMap* map, rcgf::Texture* wallTexture);
